...
"""
Classes used in exponential_smoothing.py
...
class ExponentialSmoothingToPMML:
    ...
    def get_time_value_objs(self,ts_data):
        """
        Create TimeValue objects for the input data
    ...
    def __init__(self, time_series_data, model_obj, results_obj, pmml_file_name):
        ...
        pmml = PMML(
            ...
            DataDictionary=DataDictionary(numberOfFields=n_columns, DataField=self.get_data_field_objs(time_series_data)),
            TimeSeriesModel=[TimeSeriesModel(
                modelName='simple exponential smoothing',
                functionName=function_name, bestFit=best_fit, isScorable=True,
                MiningSchema=MiningSchema(MiningField=self.get_mining_field_objs(time_series_data)),
                TimeSeries=[TimeSeries(
                    usage='original', startTime=0, endTime=n_samples - 1, interpolationMethod='none',
                    TimeValue=self.get_time_value_objs(time_series_data)
                )],
