...
"""
Functions used in skl_to_pmml.py
...
def get_entire_string(pipe0):
    """
    Create string of the Pipeline object

    Parameters:
    -----------
    pipe0 : sklearn's pipeline object

    Returns:
    --------
    pipe_container : string
        Pipeline object in string format
...
def get_mining_buildtask(pipeline):
    """
    Create MiningBuildTask object

    Parameters:
    -----------
    pipeline : sklearn's pipeline object

    Returns:
    mining_bld_task : MiningBuildTask object
    """
...
def any_in(seq_a, seq_b):
    """
    Utility function to check the common items in two python sequence

    Parameters:
    -----------
    seq_a : python sequence
        First sequence
    seq_b : python sequence
        Second sequence
...
def get_PMML_kwargs(model, derived_col_names, col_names, target_name, mining_imp_val, categoric_values):
    ...
    Returns
    -------
    algo_kwargs : python's dictionary object
...
def get_model_kwargs(model, col_names, target_name, mining_imp_val, categoric_values):
    ...
    Returns
    -------
    model_kwargs : python's dictionary object
...
def get_reg_mining_models(model, derived_col_names, col_names, target_name, mining_imp_val, categoric_values):
    """
    It returns the Mining Model element for linear models.
    ...
    -------
    mining_model : List
        Returns an mining model within a list
...
def get_reg_tab_for_reg_mining_model(model, col_names, index):
    """
    Create regression table for linear models
    ...
    --------
    reg_table : list
        Returns RegressionTable object within a list
...
def has_target(model):
    """
    Check whether the model has target
