...
#
# Generated Mon Jul 30 09:16:48 2018 by generateDS.py version 2.28a.
#
...
class AssociationModel(supermod.AssociationModel):
    ...
    14. set_AssociationRule_wrapper :
    	-----------------------------
    		Parameters :
    		------------
    			Rules :
    ...
    16. add_AssociationRule_wrapper :
    	-----------------------------
    		Parameters :
    		------------
    			value :
    ...
    18. insert_AssociationRule_at_wrapper :
    	-----------------------------------
    		Parameters :
    		------------
    			index :
...
class KohonenMap(supermod.KohonenMap):
    ...
    coord1 :
    	Describe the position of the current cluster(cluser 1) in a map with up to three dimensions, 
    coord2 :
    	Describe the position of the current cluster(cluster2) in a map with up to three dimensions, 
    coord3 :
    	Describe the position of the current cluster(cluster3) in a map with up to three dimensions, 
    Extension :"""
...
class ClusteringField(supermod.ClusteringField):
    ...
    isCenterField :
    	indicates whether the respective field is a center field,
    fieldWeight :
    	used in the comparison functions in order to compute the comparison measure,
    similarityScale :
    	the distance such that similarity becomes 0.5,
    compareFunction :
    	is a function of taking two field values and a similarityScale to define similarity/distance. It can override the general specification of compareFunction in ComparisonMeasure,
    Extension :
    	, 
    Comparisons :"""
...
class DataDictionary(supermod.DataDictionary):
    ...
    Extension :
             ,
    DataField :
     The name of a DataField must be unique from other names in the DataDictionary and, with few exceptions, unique from the names of other fields in the PMML document,
    Taxonomy :
...
class DataField(supermod.DataField):
    ...
    name :
        The name of a DataField must be unique from other names in the DataDictionary and, with few exceptions, unique from the names of other fields in the PMML document,
    displayName :
        DisplayName can be used when the application calls the PMML consumer. Once the consumer has received the parameters and matched to the MiningFields, the displayName is not relevant anymore. Only name is significant for internal processing,
    optype :
        The fields are separated into different types depending on which operations are defined on the values; this is defined by the attribute optype,
    dataType :
        This field contain data type of the feature and target name in the DataDictionary,
    mimeType :
            ,
    taxonomy :
        The optional attribute taxonomy refers to a taxonomy of values. The value is a name of a taxonomy. It describes a hierarchy of values. The attribute is only applicable to categorical fields,
    ...
    Interval :
            ,
    Value :
        Value is used to define the value ranges for fields in the DataDictionary."""
...
class Apply(supermod.Apply):
    ...
    mapMissingTo :
        used to map a missing result to the value specified by the attribute,
    defaultValue :
    ,
    ...
    Discretize :
        Takes the  input field  as input and maps values less than 0 to negative and other values to positive,
    MapValues :
...
class NearestNeighborModel(supermod.NearestNeighborModel):
    ...
    modelName :
    	element identifies the model with a unique name in the context of the PMML file,
    functionName :
    	Stores what type of problems it is ex classification or regression,
    algorithmName :
    	Stores algorithm name used in  the model,
    numberOfNeighbors :
    	Specifies K, the number of desired neighbors,
    continuousScoringMethod :
    	Specify the scoring (or combining) method based on the continuous or categorical target values of K neighbors,
    ...
    isScorable :
    	The isScorable attribute indicates whether the model is valid for scoring,
    MiningSchema :
    	list the fields that have to be provided in order to apply the model,
    Output :
    ...
    ModelExplanation :
    	,
    Targets :
    	The target values are derived from a variety of elements in the models,
    LocalTransformations :
    	,
    TrainingInstances :
    	element serves as an envelope for defining all of the training instances. It contains the definition of the fields included in the training instances as well as a table for representing the training data itself,
    ComparisonMeasure :
    	Defines the distance or similarity measure used to find the k-nearest neighbors,	
    KNNInputs :
...
class TrainingInstances(supermod.TrainingInstances):
    ...
    InstanceFields :
    	, 
    TableLocator :
    	Allows for the training data to be stored in an external table. Such a table can then be referenced by the TableLocator element which implements a kind of URL for tables, 
    InlineTable :
    	Allows for the training instances to be part of the PMML document itself. When used in k-NN models, a row in an InlineTable should contain a sequence of elements representing the input fields"""
...
class MiningField(supermod.MiningField):
    ...
    ------------
     ...
     name :
        symbolic name of field, must refer to a field in the DataDictionary,
     usageType :
     ...
     outliers :
        it shows the records which are out of trend,
     lowValue :
        Extreme low point to define outliers,
     highValue :
        Extreme high point to define outliers,
     missingValueReplacement:
        If this attribute is specified then a missing input value is automatically replaced by the given value,
     missingValueTreatment :
        this attribute just indicates how the missingValueReplacement was derived,
     invalidValueTreatment :
    ...
    2. exportAttributes_wrapper :
       --------------------------
    		...
    		*args:
    		...
    		Returns :
    		---------
    		    Returns all the exported attributes for MiningField"""
...
class MiningModel(supermod.MiningModel):
    """
    MiningModel then contains Segmentation element with a number of Segment elements as well as the attribute multipleModelMethod specifying how all the models applicable to a record should be combined.
    ...
    algorithmName :
    	Stores algorithm name used in  the model, 
    isScorable :
    	indicates whether the model is valid for scoring, 
    MiningSchema :
    	list the fields that have to be provided in order to apply the model, 
    Output :
    	describes a set of result values that can be returned from a model, 
    ...
    LocalTransformations :
    	, 
    Regression :
    	regression equation can be used to define an input transformation in another model which happens to be a TreeModel, 
    DecisionTree :
    	DecisionTree contains the essential elements of a TreeModel, 
    Segmentation :
    ...
    ModelVerification :
    	ModelVerification schema provides a dataset of model inputs and known results that can be used to verify accurate results are generated, regardless of the environment, 
    Extension :"""
...
class NaiveBayesModel(supermod.NaiveBayesModel):
    ...
    LocalTransformations :
    	, 
    BayesInputs :
    	element contains several BayesInput elements, 
    BayesOutput :
...
class BayesInputs(supermod.BayesInputs):
    ...
    Extension :
    	,
    BayesInput :
    	contains the counts pairing the discrete values of that field with those of the target field"""
...
class NeuralNetwork(supermod.NeuralNetwork):
    ...
    ------------
    modelName : 
        this stores Name of the Model ,
    functionName :
        Stores what type of problems it is ex classification or regression,
    algorithmName :
        Stores algorithm name used in  the model,
    activationFunction:
        Stores the activation function used in building model, 
    normalizationMethod :
    	,
    width :
    	,
    isScorable:
        indicates whether the model is valid for scoring,
    MiningSchema :
        Stores the MiningField in building PMML,
    Output :
    ...
    NeuralLayer :
        defines how neurons are stroed in different NeuralLayer,
    NeuralOutputs :
...
class NeuralInputs(supermod.NeuralInputs):
    ...
    numberOfInputs:
        Describe the number of input features used in Model building,
    Extension:
    ,
    NeuralInput:
        Defines how input fields are normalized so that the values can be processed in the neural network,
    ...
    6. insert_NeuralInput_at_wrapper:
       ------------------------------
            Parameters:
    		-----------
    		index:,
...
class NeuralLayer(supermod.NeuralLayer):
    """
    Neural Layer descibe the Structure of the neural network i.e. number of layer and number of neuron in each layer etc.
    ...
    numberOfNeurons :
        Indicate the number of neurons in each Layer,
    activationFunction :
        Used to determine the output of neural network like yes or no. It maps the resulting values in between 0 to 1 or -1 to 1 etc.,
    threshold :
        In threshold function, any values above or equal to a given threshold are converted to 1, while anything falling below it is converted to a 0 during activation.,
    width :
        a positive number describing the width for the radial basis function unit stored either in Neuron element or in NeuralLayer or even in NeuralNetwork.,
    altitude :
    	a positive number stored in Neuron or NeuralLayer or NeuralNetwork. The default is altitude="1.0", for that value the activation function reduces to the simple exp(-Z),
    normalizationMethod :
    	,
    Extension :
    	,
...
class NeuralOutputs(supermod.NeuralOutputs):
    ...
    numberOfOutputs :
    	Represents the number of output neurons in Output layer,
    Extension :
    	,
    NeuralOutput :
    	defines how the output of the neural network must be interpreted,"""
...
class Neuron(supermod.Neuron):
    ...
    id :
    	an identifier which must be unique in all layers
    bias :
    ...
    width :
    	a positive number describing the width for the radial basis function unit stored either in Neuron element or in NeuralLayer or even in NeuralNetwork.,
    altitude :
        a positive number stored in Neuron or NeuralLayer or NeuralNetwork.,
    Extension :
    	,
    Con :
    	Each connection Con of the element Neuron stores the ID of a node it comes from and the weight."""
...
class OutputField(supermod.OutputField):
    ...
    name :
        specifies the name of a the OutputField,
    displayName :
        ,
    optype :
        indicate admissible operations on the values. A clusterId field, for example, can have integer as its dataType, but categorical as its opType. For details, see the description of DataDictionary.,
    dataType:
        specifies the data type for the output column,
    targetField :
        must refer either to a MiningField of usage type target or a field described in Targets element,
    feature :
    ...
    rank :
        specify the rank of the feature value from the mining result that should be selected.,
    rankBasis :
        specify which criterion is used to sort the output resul,
    rankOrder :
    ...
    isMultiValued :
        indicates that the output can represent multiple output values,
    segmentId :
        applicable to MiningModels which utilize Segmentation,
    isFinalResult :
        indicate whether the result should be returned to the user or is only used as input to another OutputField that descrbes a transformed value,
    Extension :
        ,
    Decisions :
    ...
    Constant :
        used in expressions which have multiple arguments. The actual value of a constant is given by the content of the element,
    NormContinuous :
        defines how to normalize an input field by piecewise linear interpolation,
    NormDiscrete :
        refer to a certain input field define a fan-out function which maps a single input field to a set of normalized fields,
    Discretize :
...
class RegressionModel(supermod.RegressionModel):
    ...
    algorithmName :
       Can be any string describing the algorithm that was used while creating the model,
    modelType:
       Specifies the type of a regression model. The attribute modelType is for information only. It has been changed to optional and the usage is deprecated. Use functionName and normalizationMethod in order to define the computation. Use algorithmName in order to give further optional information,
    targetFieldName :
       The name of the target field (also called dependent variable). The attribute targetFieldName is for information only. It has been changed to optional and the usage is deprecated. Use usageType="target" in MiningField instead,
    normalizationMethod :
                      ,
    isScorable :
...
class VectorDictionary(supermod.VectorDictionary):
    ...
    numberOfVectors :
         The attribute numberOfVectors must be equal to the number of vectors contained in the dictionary,
    Extension :
             ,
    VectorFields :
    ...
    VectorInstance :
         The elements VectorInstance represent support vectors and are referenced by the id attribute. They do not contain the value of the target mining field."""
...
class DerivedField(supermod.DerivedField):
    ...
    ,
    optype :
    	The attribute optype is needed in order to eliminate cases where the resulting type is not known,
    dataType :
    	specifies the data type for the output column,
    datasetName :
    	, 
...
class FieldRef(supermod.FieldRef):
    ...
    field :
        ,
    mapMissingTo :
        may be used to map a missing result to the value specified by the attribute. If the attribute is not present, the result remains missing.,
    Extension :
    ,"""
...
class NormContinuous(supermod.NormContinuous):
    ...
    ,
    LinearNorm :
        defines a sequence of points for a stepwise linear interpolation function,LinearNorm must be strictly sorted by ascending value of orig."""
...
class TreeModel(supermod.TreeModel):
    """
    TreeModel in PMML allows for defining either a classification or prediction structure.
    Each Node holds a logical predicate expression that defines the rule for choosing the Node or any of the branching Nodes.
    ...
    LocalTransformations :
    	,
    Node :
    	this element is an encapsulation for either defining a split or a leaf in a tree model. Every Node contains a predicate that identifies a rule for choosing itself or any of its siblings,
    ModelVerification :
    	,
    Extension :
    	,"""
...
class Node(supermod.Node):
    ...
    score :
    	the predicted value for a record that chooses the Node,
    recordCount :
    	allow to determine the relative size of given values in a ScoreDistribution as well as the relative size of a Node when compared to the parent Node,
    defaultChild :
    ...
    Partition :
    	Optional element to provide distribution information for all records that belong to the respective Node,
    ScoreDistribution :
...
class SimplePredicate(supermod.SimplePredicate):
    ...
    field :
    	This attribute of the SimplePredicate element is the name attribute of a MiningField or a DerivedField from TransformationDictionary or LocalTransformations, 
    operator :
      ...
      greaterOrEqual	≥ , 
    value :
    	This attribute of SimplePredicate element is the information to evaluate / compare against, 
    Extension :"""
...
class SimpleSetPredicate(supermod.SimpleSetPredicate):
    ...
    field :
    	,
    booleanOperator :
    	can take one of following boolean operators: isIn, and isNotIn,
    Extension :
    	,
    Array :
    	The set of values is specified by the array in the content"""
...
class ScoreDistribution(supermod.ScoreDistribution):
    ...
    confidence :
    	This optional attribute of ScoreDistribution assigns a confidence to a given prediction class for this tree node , Confidences are similar to probabilities but more relaxed.
    ...
    probability :	
    	This optional attribute assigns a predicted probability for the given value within the node,
    Extension :"""
...
def parseLiteral(inFilename, silence=False):
    ...
    doc = None
    if not silence:
        sys.stdout.write('#from nyoka.pmml.PMML43ExtSuper import *\n\n')
