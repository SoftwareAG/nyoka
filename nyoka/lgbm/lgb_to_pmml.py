...
def lgb_to_pmml(model,derived_col_names,col_names,target_name,mining_imp_val,categoric_values,tasktype,modelPath):
    """
...
def get_segments(model, derived_col_names, col_names, target_name, mining_imp_val,categoric_values,tasktype,modelPath):
   ...
   """
    ...
    if 'LGBMClassifier' in str(model.__class__):
        segments=get_segments_for_lgbc(model, derived_col_names, col_names, target_name, mining_imp_val,categoric_values,tasktype,modelPath)
    elif 'LGBMRegressor' in str(model.__class__):
        segments=get_segments_for_lgbr(model, derived_col_names, col_names, target_name, mining_imp_val,categoric_values,tasktype,modelPath)
    return segments
...
def get_segments_for_lgbr(model, derived_col_names, feature_names, target_name, mining_imp_val,categorical_values,tasktype,modelPath):
    """
...
def get_segments_for_lgbc(model, derived_col_names, feature_names, target_name, mining_imp_val,categoric_values,tasktype,modelPath):
    """
    ...
    if model.n_classes_ == 2:
        ...
        last_segment = pml.Segment(True_=pml.True_(), id=model.n_classes_ + 1,
                                   RegressionModel=skl_to_pmml.get_regrs_models(model,oField,oField,target_name,
                                                                    mining_imp_val,categoric_values,tasktype,modelPath)[0])
        segments.append(last_segment)
    return segments
