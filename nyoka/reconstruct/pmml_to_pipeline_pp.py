...
def generate_pipeline(nyoka_pmml):
    sk_model_obj = None
    der_field = list()
    try:
        ...
        for ext in range(len(nyoka_pmml.get_MiningBuildTask().get_Extension())):
            if nyoka_pmml.get_MiningBuildTask().get_Extension()[ext].get_name() == 'preprocessingPipeline':
                pipeline_obj=nyoka_pmml.get_MiningBuildTask().get_Extension()[ext].get_value()
        import re
...
def get_regression_model(nyoka_pmml):
    cls_model = nyoka_pmml.RegressionModel[0]
    cls_model.export(outfile=open("pmmlReg.pmml", "w"), level=0)
    cat_predictors = list()
    num_predictors = list()
    funct_name = cls_model.get_functionName()
    model = cls_model.modelName
    regression_table = cls_model.get_RegressionTable()
    pmml_file = open("pmmlReg.pmml", "r")
